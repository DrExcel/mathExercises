	...
	function prepareAnswerForm() {
		...
		for (var a=0; a<aCount; a++) {
			...
			switch (aInputType) {
				...
				case 'radio':
					answerStr += aLabel+': <button type="button" id="helpBtn'+a+'" tabindex="110" style="vertical-align: top;">&#9432;</button><br>';
...
// ===========================================================================
	...
	function getSolutions() {
		var ansFlds = aufg[nr]['answerFlds'];
		var maxSolutions = Object.keys(ansFlds).length;		
		for (var i=0; i<maxSolutions; i++) {
			var aInputType = ansFlds[i]['inputType'];
			var aLabel = ansFlds[i]['label'];
			if (aInputType == 'radio') {
				var lenRadio = ansFlds[i]['options'].length;
				for (var r=0; r<lenRadio; r++) {
					var optFld = document.getElementById('sol'+aLabel+'id'+r);
					if (optFld.checked) solutions.push(optFld.value);
				}
			} else {
